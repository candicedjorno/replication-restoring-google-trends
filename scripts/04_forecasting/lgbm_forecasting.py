...
def run_lgbm_expanding_all_locations(
    geo,  
    lags, 
    exog_lags, 
    horizons, 
    ...
    for i in range(len(geo)):
        ...
        for h in horizons:
            # No exogenous
            res_noexog, rmse_noexog = lgbm_forecast_expanding_window(
                X_ts=None, Y_ts=Y_ts, horizon=h, lags=lags, exog_lags=exog_lags)
            row_noexog[f'rmse_lgbm_h{h}'] = rmse_noexog
            preds_dict['noexog'][h][state] = res_noexog['LGBM'].values
            # if date_index is None:
                # date_index = res_noexog['ds'].values
            # date_index = res_noexog['ds'].values[h:]
            date_index[h] = res_noexog['ds']

            # Clusters
            res_clusters, rmse_clusters = lgbm_forecast_expanding_window(
                X_ts=X_ts_clusters, Y_ts=Y_ts, horizon=h, lags=lags, exog_lags=exog_lags)
            row_clusters[f'rmse_lgbm_h{h}'] = rmse_clusters
            preds_dict['clusters'][h][state] = res_clusters['LGBM'].values

            # Smooth
            res_smooth, rmse_smooth = lgbm_forecast_expanding_window(
                X_ts=X_ts_smooth, Y_ts=Y_ts, horizon=h, lags=lags, exog_lags=exog_lags)
            row_smooth[f'rmse_lgbm_h{h}'] = rmse_smooth
            preds_dict['smooth'][h][state] = res_smooth['LGBM'].values

            # Detrend
            res_detrend, rmse_detrend = lgbm_forecast_expanding_window(
                X_ts=X_ts_detrend, Y_ts=Y_ts, horizon=h, lags=lags, exog_lags=exog_lags)
            row_detrend[f'rmse_lgbm_h{h}'] = rmse_detrend
            preds_dict['detrend'][h][state] = res_detrend['LGBM'].values

        results['noexog'].append(row_noexog)
        results['clusters'].append(row_clusters)
        results['smooth'].append(row_smooth)
        results['detrend'].append(row_detrend)
...
print("starting")
dfs = run_lgbm_expanding_all_locations(
    geo, 
