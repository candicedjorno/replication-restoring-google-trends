# Comparison of SARIMA and SARIMAX forecasts for horizons 0 to 3

# Using 50th percentile correlation threshold
...
start = 104 # using rolling window of past 156 weeks
min_date = '2022-10-17'
horizons = (0, 1, 2, 3)
def arima_forecast(X_ts, Y_ts, model=model):
    ...
    for n in range(start, len(dates)):
        dtrain = dates[n-start:n] # rolling window of 156 weeks
        dtest = dates[n]
...
def arima_noexog(X_ts, Y_ts, model=model):
...
def run_arima_all_locations(geo, data, save_dir, prefix, name):
    preds_h0 = pd.DataFrame()
    preds_h1 = pd.DataFrame()
    preds_h2 = pd.DataFrame()
    preds_h3 = pd.DataFrame()
...
# no exogenous
preds_h0, preds_h1, preds_h2, preds_h3, rmses_df = run_arima_all_locations(geo, clusters, save_dir, 'sarimax_010', 'noexog')

# raw clusters
preds_h0, preds_h1, preds_h2, preds_h3, rmses_df = run_arimax_all_locations(geo, clusters, save_dir, 'sarimax_010', 'clusters')

# smooth clusters
preds_h0, preds_h1, preds_h2, preds_h3, rmses_df = run_arimax_all_locations(geo, smooth, save_dir, 'sarimax_010', 'smooth')

# detrend smooth clusters
preds_h0, preds_h1, preds_h2, preds_h3, rmses_df = run_arimax_all_locations(geo, detrend, save_dir, 'sarimax_010', 'detrend')