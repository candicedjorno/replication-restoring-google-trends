...
def process_single_location_indiv(state, files, hosp, hosp_train, lags, exog_lags, horizons):
    """Process a single location for individual predictors."""
    row_indiv = {'geo': state}
    try:
        filename = [file for file in files if file.startswith(state+'_')][0]
        print(f"Processing {state}: {filename}")
        data = pd.read_csv(f'data/01_raw/individual_merged_trends/{filename}')
        
        # Clean data - remove zero columns
        ...
        for h in horizons:
            res_indiv, rmse_indiv = ada_forecast_expanding_window(
                X_ts=X_ts, Y_ts=Y_ts, horizon=h, lags=lags, exog_lags=exog_lags)
            row_indiv[f'rmse_adaboost_h{h}'] = rmse_indiv
            location_preds['indiv'][h] = res_indiv['AdaBoost'].values
            date_indices[h] = res_indiv['ds']
        ...
        return {'geo': state, 'error': str(e)}
    

def run_ada_expanding_all_locations_indiv(
    geo,  
    lags, 
    exog_lags, 
    horizons, 
    ...
    location_results = Parallel(n_jobs=n_jobs, verbose=10)(
        delayed(process_single_location_indiv)(
            state, files, hosp, hosp_train, lags, exog_lags, horizons
        ) for state in geo
    )
    ...
    for result in location_results:
        state = result['state']
        results['indiv'].append(result['indiv'])
...
def process_single_location_topics(state, files, hosp, hosp_train, lags, exog_lags, horizons):
    """Process a single location for topic predictors."""
    ...
    try:
        ...
        location_preds = {
            'topics': {h: None for h in horizons}
        }
        ...
        # Run forecasts for each horizon
        for h in horizons:
            res_topics, rmse_topics = ada_forecast_expanding_window(
                X_ts=X_ts, Y_ts=Y_ts, horizon=h, lags=lags, exog_lags=exog_lags)
            row_topics[f'rmse_adaboost_h{h}'] = rmse_topics
            location_preds['topics'][h] = res_topics['AdaBoost'].values
            date_indices[h] = res_topics['ds']
...
# Update the main topics function
def run_ada_expanding_all_locations_topics(
    geo,  
    lags, 
    exog_lags, 
    horizons, 
    ...
    # Process all locations in parallel
    location_results = Parallel(n_jobs=n_jobs, verbose=10)(
        delayed(process_single_location_topics)(
            state, files, hosp, hosp_train, lags, exog_lags, horizons
        ) for state in geo
    )
    ...
    for result in location_results:
        state = result['state']
        results['topics'].append(result['topics'])
...
# When calling your functions, specify the number of parallel jobs
dfs = run_ada_expanding_all_locations_indiv(
    geo, 
    lags=[1,2,3,4],
    exog_lags=[1,2,3,4],
    horizons=[0,1,2,3], 
...
dfs = run_ada_expanding_all_locations_topics(
    geo, 
    lags=[1,2,3,4],
    exog_lags=[1,2,3,4],
    horizons=[0,1,2,3], 
