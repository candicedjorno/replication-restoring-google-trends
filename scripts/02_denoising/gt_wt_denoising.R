...
source("scripts/02_denoising/wt_denoising.R")
...
wavelet <- "la8" # Symmlet S8
boundary <- "periodic" # periodic boundaries
nlevel <- 4 # decomposition level 
thresholds <- seq(0.1, 2, by = 0.1) # for hard thresholding
# thresholds <- 0.1 # for hard thresholding
...
result <- wt_optimal_denoising_rrmse(original, window_size, wavelet, nlevel, thresholds)
best_thresholds <- result$parameters
min_errors <- result$errors
cat("Best parameters based on train set: \n")
print(summary(best_thresholds))
...
smoothing1 <- wt_denoising_rrmse(original, window_size, wavelet, nlevel, best_thresholds, min_errors)
...
smoothing2 <- wt_denoising_rrmse(original, window_size, wavelet, nlevel, best_thresholds, min_errors)
...
combined_out <- paste0("results/denoising_results/smooth_wt_gt.csv")
